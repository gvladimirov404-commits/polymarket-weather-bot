...
let currentBalance = 0.00;          // Реален USDC баланс, ще се синхронизира автоматично
...
try {
    if (process.env.POLYMARKET_PRIVATE_KEY) {
        ...
        wallet = new Wallet(process.env.POLYMARKET_PRIVATE_KEY, provider);
...
// === Функции за проверка на баланси ===
...
async function updateBalanceFromBlockchain() {
    ...
    const matic = await getNativeBalance(wallet.address);
    if (matic !== null) {
        console.log(`⛽ MATIC баланс = ${matic.toFixed(4)} (за газ)`);
    }
}

// Пускаме синхронизация на баланса веднага и после на всеки 60 секунди
if (wallet) {
    updateBalanceFromBlockchain();
    setInterval(updateBalanceFromBlockchain, 60 * 1000);
}

// === Синхронизацията замества ръчното намаляване на баланса при сделка
// В executeTrade премахваме реда currentBalance -= betAmount
...
async function executeTrade() {
    ...
    // Използваме реалния баланс (вече актуален от синхронизацията)
    ...
    try {
        ...
        // БАЛАНСЪТ НЕ СЕ НАМАЛЯВА РЪЧНО – той ще се синхронизира от блокчейна след минута
