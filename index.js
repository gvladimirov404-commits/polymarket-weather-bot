...
// === ВРЕМЕННО ФИКСИРАН USDC БАЛАНС ===
async function getUSDCBalance(address) {
    // Тъй като Polymarket API не работи, връщаме реалния баланс ръчно
...
async function executeTrade() {
    ...
    // Проверка за MATIC (ако има грешка, просто продължаваме)
    try {
        ...
        if (maticBalance < 0.01) {
            ...
            await sendToTelegram(`⚠️ Ботът спря: няма MATIC за газ! Моля, депозирайте поне 0.5 MATIC.`);
            isBotActive = false;
