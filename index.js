...
async function runBacktest(days = 7) {
    ...
    let wins = 0, losses = 0;
    let trades = [];
    const cyclesPerDay = currentStrategy === STRATEGIES.BTC_5MIN ? 288 : 24;
    ...
    console.log(`📊 BACKTEST: Win Rate ${winRate.toFixed(1)}% | P&L: ${totalPnL >= 0 ? '+' : ''}$${totalPnL.toFixed(2)}`);
    
    backtestRunning = false;
    return { winRate, totalPnL, wins, losses, avgWin, avgLoss, finalBalance: simulatedBalance };
}

// ========== CLOB ИНИЦИАЛИЗАЦИЯ (напълно запазена) ==========
...
async function runBacktest() {
    ...
    btn.disabled = true;
    btn.textContent = '⏳ Backtest в ход...';
    const response = await fetch('/backtest');
    const result = await response.json();
    const div = document.getElementById('backtestResult');
    div.innerHTML = '<b>📊 Backtest резултати (7 дни):</b><br>' +
        '🏆 Win Rate: ' + result.winRate.toFixed(1) + '% (' + result.wins + '/' + (result.wins + result.losses) + ')<br>' +
        '💰 Краен баланс: $' + result.finalBalance.toFixed(2) + '<br>' +
        '📈 Обща печалба: ' + (result.totalPnL >= 0 ? '+' : '') + '$' + result.totalPnL.toFixed(2) + '<br>' +
        '📊 Средна печалба: $' + (result.avgWin || 0).toFixed(4) + ' | Средна загуба: $' + (result.avgLoss || 0).toFixed(4);
    btn.disabled = false;
    btn.textContent = '📊 ПУСНИ BACKTEST';
}
...
async function loadTrades() {
    ...
    if (!trades.length) { container.innerHTML = '<div>Няма изпълнени сделки</div>'; return; }
    container.innerHTML = trades.slice(0, 30).map(t => '<div style="padding:8px;border-bottom:1px solid #2a2a30">' +
        '<b>' + new Date(t.time).toLocaleTimeString() + '</b> | ' +
        '<span class="' + (t.direction === 'UP' ? 'positive' : 'negative') + '">' + (t.strategy || t.direction || '-') + '</span> | ' +
        '$' + (t.amount || 0).toFixed(2) + ' | ' +
        (t.status || '-') +
        (t.profit ? ' | <span class="' + (t.profit > 0 ? 'positive' : 'negative') + '">' + (t.profit > 0 ? '+' : '') + t.profit.toFixed(4) + '</span>' : '') +
        '</div>').join('');
}
...
app.post('/switch-strategy', (req, res) => {
    ...
    if (strategy === STRATEGIES.HEDGE || strategy === STRATEGIES.BTC_5MIN) {
        ...
        settings = { ...DEFAULT_SETTINGS[currentStrategy], ...settings };
        saveSettings();
        console.log(`🔄 Смяна на стратегия: ${currentStrategy}`);
...
// ========== ОСНОВЕН ТЪРГОВСКИ ЦИКЪЛ ==========
async function mainTradingLoop() {
    if (currentStrategy === STRATEGIES.BTC_5MIN) {
        await executeBTC5MinTrade();
    } else {
        await executeHedgeTrade();
    }
}

function resetForNewCycle() {
    if (currentStrategy === STRATEGIES.BTC_5MIN) {
        const secondsInWindow = Math.floor((Date.now() / 1000) % 300);
        if (secondsInWindow === 0) marketStartPrice = 0;
    }
}
...
async function start() {
    loadSettings();
    console.log('🚀 HyperMeteo V4 - Multi-Strategy Bot');
    console.log(`📋 Текуща стратегия: ${currentStrategy === STRATEGIES.HEDGE ? 'Хедж 70/15/15' : '5-min BTC'}`);
    console.log('=============================================');
    await initClob();
    await updateRealBalance();
    setInterval(mainTradingLoop, 1000);
    setInterval(resetForNewCycle, 1000);
