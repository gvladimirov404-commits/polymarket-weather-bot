...
// Контракт на USDC в мрежата Polygon
...
let currentBalance = 0.00;      // ще се обновява от блокчейн
let currentBetSize = 5.00;      // максимален залог (можеш да промениш)
...
// === Четене на реален USDC баланс от блокчейна ===
...
async function updateBalance() {
    ...
    if (usdc !== null) {
        currentBalance = usdc;
        console.log(`💰 Реален USDC баланс = $${currentBalance.toFixed(2)}`);
    }
}

// === Четене на MATIC баланс (само за информация) ===
...
async function executeTrade() {
    ...
    // Актуализираме баланса преди сделка (за по-голяма точност)
    await updateBalance();
    ...
    // Залог = min(максимален залог, 25% от реалния баланс)
    ...
    console.log(`📊 Залог: $${betAmount.toFixed(2)} (${(betAmount/currentBalance*100).toFixed(1)}% от баланса)`);
