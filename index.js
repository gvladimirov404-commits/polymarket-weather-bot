...
// МАТЕМАТИЧЕСКА ФУНКЦИЯ ЗА ХЕДЖИРАНЕ (СТРАТЕГИЯ ПРЕЗ ЕДНА ПОЗИЦИЯ)
function calculateHedging(totalBet, mainPrice = 0.70, hedgePrice = 0.02) {
    const mainBudget = totalBet * 0.85;
    const hedgeBudget = totalBet * 0.15;

    const mainShares = mainBudget / mainPrice;
    const hedgeShares = hedgeBudget / hedgePrice;

    const payoutIfMainWins = mainShares;
    const payoutIfHedgeWins = hedgeShares;

    const netProfitMain = payoutIfMainWins - totalBet;
    const netProfitHedge = payoutIfHedgeWins - totalBet;

    return {
        main: { budget: mainBudget, price: mainPrice, shares: Math.floor(mainShares), netProfit: netProfitMain },
        hedge: { budget: hedgeBudget, price: hedgePrice, shares: Math.floor(hedgeShares), netProfit: netProfitHedge }
    };
}

async function sendToTelegram(message) {
    if (!TG_TOKEN || !TG_CHAT_ID) {
        console.log("Липсват Telegram настройки in променливите.");
        return;
    }
    try {
...
app.post('/execute-trade', async (req, res) => {
    ...
    const hedgeCalc = calculateHedging(currentBetSize, 0.70, 0.02);

    const alertMsg = `
🚀 <b>НОВ СИГНАЛ С АВТОХЕДЖИРАНЕ</b>
<b>Режим:</b> ${mode}
...
🔹 <b>ОСНОВЕН ЗАЛОГ (85%):</b>
  • Бюджет: $${hedgeCalc.main.budget.toFixed(2)} USDC
  • Цена договор: $${hedgeCalc.main.price.toFixed(2)}
  • Купени договори: ${hedgeCalc.main.shares} бр.
  • Чиста печалба при успех: +$${hedgeCalc.main.netProfit.toFixed(2)} USDC

🛡️ <b>ЗАСТРАХОВКА ПРЕЗ ЕДНА (15%):</b>
  • Бюджет: $${hedgeCalc.hedge.budget.toFixed(2)} USDC
  • Цена договор: $${hedgeCalc.hedge.price.toFixed(2)} (Шанс 50х)
  • Купени договори: ${hedgeCalc.hedge.shares} бр.
  • <b>Чиста печалба при аномалия: +$${hedgeCalc.hedge.netProfit.toFixed(2)} USDC</b>

<b>Статус такса:</b> ${feeStatus}
<b>Действие:</b> Изпращане на разделени ордери към Polymarket...`;
