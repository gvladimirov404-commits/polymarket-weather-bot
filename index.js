...
// Инициализация на ethers provider (само за MATIC баланс и газ)
...
// === НОВА ФУНКЦИЯ ЗА ЧЕТЕНЕ НА USDC БАЛАНС ДИРЕКТНО ОТ POLYMARKET API ===
async function getUSDCBalance(address) {
    try {
        // Polymarket API за баланс (достъпно за всеки адрес)
        const response = await axios.get(`https://data-api.polymarket.com/accounts/${address}/balances?asset=USDC`);
        if (response.data && response.data.balance !== undefined) {
            return parseFloat(response.data.balance);
        }
        return 0;
    } catch (err) {
        console.error("❌ Грешка при зареждане на USDC баланс от Polymarket:", err.message);
        return 0;
    }
}

// Четене на MATIC баланс (за газ)
...
async function executeTrade() {
    ...
    // Проверка за MATIC (газ)
    const maticBalance = await getMATICBalance(account.address);
    if (maticBalance < 0.01) {
        console.log(`⚠️ Недостатъчен MATIC за газ: ${maticBalance.toFixed(4)} MATIC`);
        await sendToTelegram(`⛔ Ботът спря: няма MATIC за газ! Моля, депозирайте поне 0.5 MATIC.`);
        isBotActive = false;
