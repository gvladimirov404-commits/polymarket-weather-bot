...
let isBotActive = true;      // <-- ботът е активен по подразбиране
...
// === ФИКСИРАН USDC БАЛАНС (докато API-то не проработи) ===
async function getUSDCBalance(address) {
    // Връща реалния баланс, който знаем от Polymarket
    console.log(`💰 Ръчно зададен USDC баланс = $17.93`);
    return 17.93;
}
...
async function executeTrade() {
    ...
    // Проверка за MATIC – само предупреждение, НЕ спира бота!
    try {
        const maticBalance = await getMATICBalance(account.address);
        if (maticBalance < 0.01) {
            console.log(`⚠️ ВНИМАНИЕ: Нисък MATIC баланс (${maticBalance.toFixed(4)} MATIC). Може да няма достатъчно за газ.`);
            await sendToTelegram(`⚠️ Нисък MATIC баланс: ${maticBalance.toFixed(4)} MATIC. Моля, депозирайте поне 0.5 MATIC.`);
            // isBotActive остава true – продължаваме!
        }
    } catch (err) {
        console.log("⚠️ Не мога да проверя MATIC баланса, но продължавам");
    }
