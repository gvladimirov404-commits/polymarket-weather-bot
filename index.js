...
const COOLDOWN_TIME = 60 * 1000;   // 1 минута
...
let currentBalance = 0.00;         // ще се зареди от Polymarket API
...
(async () => {
    try {
        ...
        // 2. Временен клиент за извличане на L2 credentials
        ...
        // 4. Зареждаме реалния USDC баланс от Polymarket API
        await updateBalance();
...
// ========== ЧЕТЕНЕ НА USDC БАЛАНС ОТ POLYMARKET API ==========
async function getUSDCBalanceFromPolymarket(address) {
    try {
...
async function updateBalance() {
    if (!account) return;
    const usdc = await getUSDCBalanceFromPolymarket(account.address);
    if (usdc !== null) {
        currentBalance = usdc;
        console.log(`💰 Реален USDC баланс (от Polymarket): $${currentBalance.toFixed(2)}`);
    }
}
