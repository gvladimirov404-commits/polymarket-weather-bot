...
// 🧠 ОБНОВЕНА МАТЕМАТИКА: ДВОЕН ХЕДЖ (70% / 15% / 15%)
function calculateDoubleHedging(totalBet, mainPrice = 0.70, hedgeUpPrice = 0.02, hedgeDownPrice = 0.02) {
    // Разделяне на капитала
    const mainBudget = totalBet * 0.70;      // 70% за центъра
    const hedgeUpBudget = totalBet * 0.15;   // 15% за горна застраховка
    const hedgeDownBudget = totalBet * 0.15; // 15% за долна застраховка

    // Изчисляване на количества договори (Shares)
    ...
    // Чиста печалба при всеки един от трите сценария (Бруто - Общия залог)
...
app.post('/execute-trade', async (req, res) => {
    ...
    const strategy = "Двоен Хедж (Крайности)";

    // Извикване на новата математика (Тестови цени: Основен $0.70, Хеджове $0.02)
    const hedgeCalc = calculateDoubleHedging(currentBetSize, 0.70, 0.02, 0.02);

    const alertMsg = `
🚀 <b>НОВ СИГНАЛ: ДВОЕН АВТОХЕДЖ</b>
<b>Режим:</b> ${mode}
...
🔹 <b>ОСНОВЕН ЗАЛОГ - ЦЕНТЪР (70%):</b>
  ...
  • Чиста печалба при кротко време: <b>+$${hedgeCalc.main.netProfit.toFixed(2)} USDC</b>

🛡️ <b>ХЕДЖ НАГОРЕ - ЖЕГА (15%):</b>
  • Бюджет: $${hedgeCalc.hedgeUp.budget.toFixed(2)} USDC
  • Цена: $${hedgeCalc.hedgeUp.price.toFixed(2)} | Договори: ${hedgeCalc.hedgeUp.shares} бр.
...
❄️ <b>ХЕДЖ НАДОЛУ - СТУД (15%):</b>
  ...
  • Цена: $${hedgeCalc.hedgeDown.price.toFixed(2)} | Договори: ${hedgeCalc.hedgeDown.shares} бр.
  • Чиста печалба при рязък студ: <b>+$${hedgeCalc.hedgeDown.netProfit.toFixed(2)} USDC</b>

<b>Статус такса:</b> ${feeStatus}
<b>Действие:</b> Изпращане на 3 отделни ордера към Polymarket...`;
