...
(async () => {
    try {
        ...
        // Стартираме автоматичната търговия СЛЕД като всичко е готово
        startAutoTrade();
...
function startAutoTrade() {
    console.log("🚀 Стартиране на автоматична търговия...");
    setInterval(async () => {
        if (isBotActive && isInitialized && clobClient) {
            console.log("🔄 Изпълнявам автоматична проверка...");
            const result = await executeTrade();
            if (!result.success) {
                console.log("⏭️ Авто-трейд пропуснат:", result.error);
            }
        } else {
            console.log("⏳ Ботът не е готов за търговия");
        }
    }, AUTO_TRADE_INTERVAL);
}
