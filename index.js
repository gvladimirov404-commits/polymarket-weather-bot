...
app.post('/execute-trade', async (req, res) => {
    ...
    // РЕАЛНА ЛОГИКА ЗА ЛИМИТИРАНИ ПОРЪЧКИ
...
app.get('/', (req, res) => {
    ...
    <head>
        ...
        <style>
            ...
            .m-btn { padding: 12px 5px; background: #1a1a1a; border: 1px solid #333; color: #777; cursor: pointer; border-radius: 8px; font-size: 10px; font-weight: bold; text-align: center; transition: 0.2s; }
    ...
    <body>
        ...
        <script>
            ...
            const translations = {
                bg: { bal: "ТЕКУЩ БАЛАНС (USDC)", ref: "РЕФЕРАЛ ЗА ОТСТЪПКА", btn: "ИЗПЪЛНИ СДЕЛКА СЕГА", log: "Режим: " },
                en: { bal: "CURRENT BALANCE (USDC)", ref: "REFERRAL DISCOUNT", btn: "EXECUTE TRADE NOW", log: "Mode: " },
                ru: { bal: "ТЕКУЩИЙ БАЛАНС (USDC)", ref: "РЕФЕРАЛ ДЛЯ СКИДКИ", btn: "ВЫПОЛНИТЬ СДЕЛКУ СЕЙЧАС", log: "Режим: " }
            };
            ...
            async function startTrade() {
                const l = document.getElementById('lang').value;
                addLog(l === 'bg' ? "Изпращане на поръчка..." : "Sending order...");
                ...
                try {
                    const res = await fetch('/execute-trade', {
                        ...
                        body: JSON.stringify({ 
                            mode: currentMode, 
                            lang: l,
                            userId: "USER_ID_FROM_TELEGRAM"
                        })
                    });
                    if (res.ok) addLog("УСПЕХ: Сигналът е в Телеграм.");
