...
try {
    if (process.env.POLYMARKET_PRIVATE_KEY) {
        ...
        console.log("✅ Polymarket CLOB Client ready");
    }
...
async function closePosition(tokenId, shares, side) {
    ...
    try {
        await clobClient.createOrder({
            ...
            size: shares,
            price: 0.01 // маркет симулация – в реалност трябва да вземеш книгата
        });
        return true;
...
// ==================== ОСНОВНА ТЪРГОВСКА ФУНКЦИЯ (автоматична) ====================
...
// Периодично затваряне на печеливши/губещи позиции
setInterval(async () => {
    if (isBotActive) {
        const marketData = await refreshLiquidMarkets();
...
// ==================== API ENDPOINTS (съвместими с твоя UI) ====================
...
// HTML UI (копирай твоя оригинален HTML – той остава 100% същият, само добави refresh на статуса)
app.get('/', (req, res) => {
    res.send(`
    <!DOCTYPE html>
    <html lang="bg">
