...
async function getRealWeatherForecasts(lat, lon) {
  ...
  try {
    ...
    if (data.hourly && data.hourly.temperature_2m.length > 0) {
      ...
      // Симулираме малки разлики между моделите (в реалност Open-Meteo дава само един модел)
      return {
        gfs: tempAt12,
...
function consensusTemperature(models) {
  ...
  // Увереност: по-малка вариация = по-висока увереност, но ограничаваме максимум 95%
            ...
            async function testTrade() {
                ...
                try {
                    ...
                    if (data.decision !== 'hold') {
                        lastTradeDiv.innerHTML = \`✅ Решение: \${data.decision === 'up' ? '📈 КУПУВА НАГОРЕ' : '📉 КУПУВА НАДОЛУ'}<br>📝 Причина: \${data.reason}<br>🌡️ Консенсус: \${data.consensus}°C | Увереност: \${data.confidence}%\`;
                    } else {
...
app.post('/trade/decision', async (req, res) => {
  ...
  // Коригирана логика за решение
  if (confidence > 70) {
    if (consensus > 15.5 && clampedPrice < 0.85) {
      decision = 'up';
      reason = `Консенсус ${consensus.toFixed(1)}°C > 15.5°C, увереност ${confidence.toFixed(0)}%`;
      size = userState.perTradeBudget;
    } 
    else if (consensus < 14.5 && clampedPrice > 0.15) {
      decision = 'down';
      reason = `Консенсус ${consensus.toFixed(1)}°C < 14.5°C, увереност ${confidence.toFixed(0)}%`;
      size = userState.perTradeBudget;
    }
    else {
      reason = `Консенсусът е в неутрална зона (14.5-15.5°C) или цената е крайна (${clampedPrice.toFixed(2)}).`;
    }
  } else {
