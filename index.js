...
let account;
let signer;
let provider;   // за ethers (четене на баланси)

// ==================== ИНИЦИАЛИЗАЦИЯ НА CLOB (viem) ====================
try {
    ...
    console.log('✅ ClobClient инициализиран (без L2 ключове още)');

    // Автоматично създаване или извличане на L2 ключове
    (async () => {
        try {
            const creds = await clobClient.createOrDeriveApiKey();
            console.log('✅ L2 ключовете са готови (или вече съществуват)');
            // Може да запазите creds в лог, но не са задължителни за работа
            if (creds.apiKey) console.log(`   API Key: ${creds.apiKey.substring(0, 10)}...`);
        } catch (err) {
            console.error('❌ Грешка при L2 ключове:', err.message);
        }
    })();
} catch (e) {
...
async function getUSDCBalance(address) {
    ...
    try {
        ...
        console.error('Грешка при USDC баланс:', err.message);
        return null;
    }
}
...
async function getNativeBalance(address) {
    ...
    try {
        ...
        console.error('Грешка при MATIC баланс:', err.message);
        return null;
    }
}
