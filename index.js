...
async function executeTrade() {
    ...
    try {
        console.log(`🚀 Сделка на: ${marketData.title}`);
        ...
        if (marketData.tokens[0]) {
            ...
            console.log(`✅ 1/3: ${hedgeCalc.main.shares} shares at ${marketData.prices.main}`);
        }
        if (marketData.tokens[1]) {
            ...
            console.log(`✅ 2/3: ${hedgeCalc.hedgeUp.shares} shares at ${marketData.prices.hedgeUp}`);
        }
        if (marketData.tokens[2]) {
            ...
            console.log(`✅ 3/3: ${hedgeCalc.hedgeDown.shares} shares at ${marketData.prices.hedgeDown}`);
        }

        lastTradeTimestamp = Date.now();
        currentBalance -= betAmount;
        
        await sendToTelegram(`✅ ${betAmount} USDC на ${marketData.title}`);
