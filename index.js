...
async function executeTrade() {
    ...
    try {
        console.log(`🚀 СДЕЛКА: ${marketData.title}`);
        console.log(`💰 Залог: $${betAmount} USDC`);
        console.log(`📊 Yes цена: ${marketData.prices.main} | No цена: ${marketData.prices.hedgeUp}`);
        ...
        if (marketData.tokens[0]) {
            ...
            console.log(`✅ КУПИ YES: ${hedgeCalc.main.shares} shares at ${marketData.prices.main}`);
        }
        if (marketData.tokens[1]) {
            ...
            console.log(`✅ КУПИ NO: ${hedgeCalc.hedgeUp.shares} shares at ${marketData.prices.hedgeUp}`);
        }

        lastTradeTimestamp = Date.now();
        currentBalance -= betAmount;
        
        await sendToTelegram(`✅ Сделка ${betAmount} USDC\nПазар: ${marketData.title}\nYes: ${marketData.prices.main}\nNo: ${marketData.prices.hedgeUp}`);
...
// Автоматична търговия на всеки 1 минута
setInterval(async () => {
    if (isBotActive) {
        console.log("⏰ Автоматично сканиране...");
        const result = await executeTrade();
        if (!result.success) console.log("⏭️ Пропусната сделка:", result.error);
    }
}, AUTO_TRADE_INTERVAL);
