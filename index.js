...
// USDC contract on Polygon (за автоматично четене на баланс)
...
let currentBalance = 0.00;      // автоматично синхронизиран USDC баланс
...
let account;
let signer;
let provider;   // ще се използва за баланс (ethers JsonRpcProvider)

try {
    if (!process.env.POLYMARKET_PRIVATE_KEY) {
        throw new Error('POLYMARKET_PRIVATE_KEY not found in .env');
    }
    // 1. Създаваме account и signer чрез viem
    ...
    console.log('✅ CLOB V2 Client ready (viem)');

    // 4. Подготвяме JsonRpcProvider за четене на баланси (ethers е все още инсталиран)
    const { providers, Contract } = require('ethers');
    provider = new providers.JsonRpcProvider('https://polygon-rpc.com');
} catch (e) {
    console.error('❌ CLOB init error:', e.message);
}

// Firebase (опционално) – ако има FIREBASE_CONFIG, свързваме
let db = null;
...
// === Функции за автоматично синхронизиране на USDC баланс ===
async function getUSDCBalance(address) {
    ...
    try {
        ...
        console.error('Грешка при четене на USDC баланс:', err.message);
        return null;
    }
}
...
async function getNativeBalance(address) {
    ...
    try {
        ...
        console.error('Грешка при четене на MATIC баланс:', err.message);
        return null;
    }
}

async function updateBalanceFromBlockchain() {
    if (!account) return;
    const usdc = await getUSDCBalance(account.address);
    if (usdc !== null) {
        currentBalance = usdc;
        console.log(`💰 Авто-синхронизация: USDC баланс = $${currentBalance.toFixed(2)}`);
    }
    const matic = await getNativeBalance(account.address);
    if (matic !== null) {
        console.log(`⛽ MATIC баланс = ${matic.toFixed(4)} (за газ)`);
    }
}

if (account) {
    updateBalanceFromBlockchain();
    setInterval(updateBalanceFromBlockchain, 60 * 1000);
}
...
// === Проверка за TP/SL (ако има Firebase) ===
...
// === Основна функция за изпълнение на сделка ===
...
// Проверка за TP/SL на всяка минута
setInterval(async () => {
    if (isBotActive) {
        const marketData = await refreshLiquidMarkets();
