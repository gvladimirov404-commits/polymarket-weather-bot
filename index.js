...
async function loadTrades() {
    ...
    tbody.innerHTML = trades.slice(0,30).map(t=>'<td><td>'+new Date(t.time).toLocaleTimeString()+'</td><td>'+(t.strategy||'-')+'</td><td>$'+(t.amount||0).toFixed(2)+'</td><td class="'+(t.mode==='REAL'?'positive':'negative')+'">'+(t.mode||'TEST')+'</td><td>'+(t.status||'-')+'</td></tr>').join('');
}
