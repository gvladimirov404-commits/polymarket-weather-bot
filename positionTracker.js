// positionTracker.js
// Следи позициите на бота и затваря при TP (15%) или SL (8%)

let activePositions = new Map(); // tokenId -> { avgPrice, shares, conditionId }

async function loadOurTrades(clobClient) {
    if (!clobClient) return false;
    try {
        // Извикваме getBuilderTrades() без параметри, но обработваме празен резултат
        let trades = [];
        try {
            ...
            if (response && Array.isArray(response)) {
                ...
                console.log("ℹ️ Няма предишни сделки или API върна празен отговор.");
                return true;
            }
        } catch (err) {
        ...
        if (trades.length === 0) {
            console.log("ℹ️ Няма отворени позиции.");
            return true;
        }
        ...
        for (const [tokenId, data] of tokenMap.entries()) {
            activePositions.set(tokenId, data);
            console.log(`📌 Следя позиция: ${tokenId.substring(0, 12)}..., avg: ${data.avgPrice}, shares: ${data.shares}`);
        }
        return true;
    } catch (err) {
