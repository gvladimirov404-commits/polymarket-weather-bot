...
async function refreshLiquidMarkets() {
    ...
    try {
        ...
        // 1. Опитваме първо да намерим метеорологични пазари
        ...
        if (weatherMarkets.length > 0) {
            // Сортираме метео пазарите по обем
            ...
            // 2. Ако няма метео, взимаме най-ликвидния пазар от всички
            markets.sort((a, b) => (b.volume || 0) - (a.volume || 0));
            selectedMarket = markets[0];
            console.log(`📊 Няма метео пазари. Избран най-ликвидният пазар: ${selectedMarket.question}`);
        }

        const volume = selectedMarket.volume || selectedMarket.volume24hr || 0;
        if (volume < MIN_LIQUIDITY) {
            console.log(`⚠️ Ликвидността ($${volume}) е под минималната ($${MIN_LIQUIDITY})`);
        ...
        if (selectedMarket.clobTokenIds) {
            if (Array.isArray(selectedMarket.clobTokenIds)) {
                tokens = selectedMarket.clobTokenIds;
            } else if (typeof selectedMarket.clobTokenIds === 'string') {
                try {
                    tokens = JSON.parse(selectedMarket.clobTokenIds);
                } catch(e) {}
            }
        }
        if (tokens.length === 0) {
            return { success: false, error: "Няма токени за търговия" };
        }
